...
echo "<pre>Setup by kianzo | JualSSH.com | @arieonline | 7946F434</pre>" > /home/vps/public_html/index.html
...
useradd -M -s /bin/false kianzo
echo "kianzo:$PASS" | chpasswd
...
# setting port ssh
sed -i '/Port 22/a Port 143/a Port 69' /etc/ssh/sshd_config
...
# info
clear
echo "ophreakerz.in | @ophreakerz | kianzo | XXXXXXX" | tee log-install.txt
echo "===============================================" | tee -a log-install.txt
echo ""  | tee -a log-install.txt
echo "Service"  | tee -a log-install.txt
echo "-------"  | tee -a log-install.txt
echo "OpenVPN  : TCP 1194 (client config : http://$MYIP/client.tar)"  | tee -a log-install.txt
echo "OpenSSH  : 22, 143, 69"  | tee -a log-install.txt
...
echo "---------------"  | tee -a log-install.txt
echo "User     : kianzo"  | tee -a log-install.txt
